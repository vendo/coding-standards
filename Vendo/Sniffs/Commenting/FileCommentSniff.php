...
class Vendo_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    protected $tags = array(
					    //@scmod category is not required
                       'category'   => array(
                                        'required'       => false,
                       ...
                       'license'    => array(
                                        'required'       => true,
                                        'allow_multiple' => false,
                                        'order_text'     => 'follows @copyright (if used) or @package',
                                       ),
                       'version'    => array(
                                        'required'       => false,
                                        'allow_multiple' => false,
    ...
    protected function processTags($commentStart, $commentEnd)
    {
        ...
        foreach ($this->tags as $tag => $info) {
            ...
            if ($info['required'] === true && in_array($tag, $foundTags) === false) {
				// @scmod - hack to allow copyright and package 
				//          to only be required once (in a file), 
				//          instead of in both file and class docs
				// There are much, much better ways of doing this,
				// feel free to implement one.
				if ( ('copyright' == $tag || 'package' == $tag) && 
					$this instanceof 
						Vendo_Sniffs_Commenting_ClassCommentSniff ) 
				{
					continue;
				}
