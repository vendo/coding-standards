 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Vendo_Sniffs_NamingConventions_ValidFunctionNameSniff extends PHP_CodeSniffer_Standards_AbstractScopeSniff
{
    ...
    protected function processTokenWithinScope(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $currScope)
    {
		...
		// @scmod - protected methods may have an underscore
		// If it's not a private method, 
		// it must not have an underscore on the front.
        if (($isPrivate === false && $isProtected === false) && $scopeSpecified === true && $methodName{0} === '_') {
            $error = ucfirst($scope)." method name \"$className::$methodName\" must not be prefixed with an underscore";
            $phpcsFile->addError($error, $stackPtr);
            return;
        }

		// @scmod Replaced camel caps checker below with Kohana version
        if ( ! self::isKohanaMethodName($methodName)) {
            if ($scopeSpecified === true) {
                $error = ucfirst($scope)." method name \"$className::$methodName\" is not in Kohana format";
            } else {
                $error = "Method name \"$className::$methodName\" is not in Kohana format";
            }
            $phpcsFile->addError($error, $stackPtr);
            return;
        }
	...
	public static function isKohanaMethodName($methodName)
	{
		if (preg_match('/^[_a-z0-9]+$/', $methodName)) {
			return true;
		}
